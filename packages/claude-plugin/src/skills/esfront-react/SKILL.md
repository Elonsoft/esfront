...
- [Components](references/components.md) — what every exported component is for, and which ones are deprecated.
- [Hooks](references/hooks.md) — what every exported hook is for, and which ones are deprecated.

The references name and describe the API; they do not carry props or signatures. Read the relevant one to find the right
component or hook, then read its declarations in `node_modules/@esfront/react/lib` for the props it takes and the values
it returns. Do not guess prop names.
...
If a component or hook is not on these lists, it does not come from `@esfront/react` — check the project's own code
instead.

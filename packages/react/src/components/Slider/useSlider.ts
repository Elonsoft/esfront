...
export function useSlider(parameters: UseSliderParameters): UseSliderReturnValue {
  ...
  const createHandleMouseDown = (otherHandlers: EventHandlers) => (event: React.MouseEvent<HTMLSpanElement>) => {
  ...
  const getRootProps = <ExternalProps extends Record<string, unknown> = any>(
  ...
  const getThumbProps = <ExternalProps extends Record<string, unknown> = any>(
    externalProps: ExternalProps = {} as ExternalProps
  ...
  const getHiddenInputProps = <ExternalProps extends Record<string, unknown> = any>(
    externalProps: ExternalProps = {} as ExternalProps

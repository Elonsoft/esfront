...
export function getSizeMultiplier(wrapperComponent: HTMLDivElement, equalToMove: boolean): number {
...
export function handleCalculateVelocity(contextInstance: ReactZoomPanPinchContext, position: PositionType): void {
...
export function handleVelocityPanning(contextInstance: ReactZoomPanPinchContext): void {
  ...
  const finalAnimationTime = Math.max(moveAnimationTime, alignAnimationTime ?? 0);

  const paddingValueX = getPaddingValue(contextInstance, sizeX ?? 0);
  const paddingValueY = getPaddingValue(contextInstance, sizeY ?? 0);
  ...
  if (animationType && alignAnimationTime) {
    handleSetupAnimation(contextInstance, animationType, finalAnimationTime, (step: number) => {

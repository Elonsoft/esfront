...
export const isPanningAllowed = (contextInstance: ReactZoomPanPinchContext): boolean => {
...
export const handlePanningSetup = (contextInstance: ReactZoomPanPinchContext, event: MouseEvent): void => {
...
export const handleTouchPanningSetup = (contextInstance: ReactZoomPanPinchContext, event: TouchEvent): void => {
  const { touches } = event;
  const { positionX, positionY } = contextInstance.transformState;
...
export function handlePanToBounds(
  ...
  const { maxPositionX, minPositionX, maxPositionY, minPositionY } = contextInstance.bounds;

  const xChanged = positionX > maxPositionX || positionX < minPositionX;
  const yChanged = positionY > maxPositionY || positionY < minPositionY;

  const mousePosX = positionX > maxPositionX ? wrapperComponent.offsetWidth : contextInstance.setup.minPositionX || 0;
  const mousePosY = positionY > maxPositionY ? wrapperComponent.offsetHeight : contextInstance.setup.minPositionY || 0;
...
export const getPaddingValue = (contextInstance: ReactZoomPanPinchContext, size: number): number => {

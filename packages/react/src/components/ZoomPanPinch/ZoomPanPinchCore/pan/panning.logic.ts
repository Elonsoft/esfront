...
import { handleCalculateVelocity, handleVelocityPanning } from './velocity.logic';
...
export function handlePanningStart(contextInstance: ReactZoomPanPinchContext, event: MouseEvent | TouchEvent): void {
...
export function handleAlignToBounds(contextInstance: ReactZoomPanPinchContext): void {
  const { scale } = contextInstance.transformState;
...
export function handlePanning(contextInstance: ReactZoomPanPinchContext, clientX: number, clientY: number): void {
...
export function handlePanningEnd(contextInstance: ReactZoomPanPinchContext): void {
  if (contextInstance.isPanning) {
    ...
    const shouldAnimate = !velocityDisabled && velocity && velocity?.total > 0.1 && isZoomed;

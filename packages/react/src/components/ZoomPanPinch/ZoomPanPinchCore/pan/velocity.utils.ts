...
export const isVelocityCalculationAllowed = (contextInstance: ReactZoomPanPinchContext): boolean => {
...
export const isVelocityAllowed = (contextInstance: ReactZoomPanPinchContext): boolean => {
...
export function getVelocityMoveTime(contextInstance: ReactZoomPanPinchContext, velocity: number): number {
  const { velocityAnimation } = contextInstance.setup;
  const { equalToMove, animationTime, sensitivity } = velocityAnimation;

  if (equalToMove) {
    return (animationTime ?? 0) * velocity * (sensitivity ?? 0);
  }
...
export function getVelocityPosition(
  ...
  if (limitToBounds) {
    if (startPosition > maxPosition && currentPosition > maxPosition) {
      const calculatedPosition = maxPosition + (newPosition - maxPosition) * step;
    ...
    if (startPosition < minPosition && currentPosition < minPosition) {
      const calculatedPosition = minPosition + (newPosition - minPosition) * step;

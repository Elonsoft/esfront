...
export const zoomIn =
  ...
  (step = 0.5, animationTime = 300, animationType: keyof typeof animations = 'easeOut'): void => {
...
export const resetTransform =
  (contextInstance: ReactZoomPanPinchContext) =>
  (animationTime = 200, animationType: keyof typeof animations = 'easeOut'): void => {
    resetTransformations(contextInstance, animationTime, animationType);
  };

export const centerView =
  (contextInstance: ReactZoomPanPinchContext) =>
  (scale?: number, animationTime = 200, animationType: keyof typeof animations = 'easeOut'): void => {
    const { transformState, wrapperComponent, contentComponent } = contextInstance;

    if (wrapperComponent && contentComponent) {
      const targetState = getCenterPosition(scale || transformState.scale, wrapperComponent, contentComponent);
...
export const zoomToElement =
  ...
  (
    ...
    const target: HTMLElement | null = typeof node === 'string' ? document.getElementById(node) : node;

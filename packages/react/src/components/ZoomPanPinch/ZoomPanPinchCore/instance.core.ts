...
import { handleWheelStart, handleWheelStop, handleWheelZoom } from './wheel/wheel.logic';
...
export class ZoomPanPinch {
  ...
  public onChangeCallbacks: Set<(ctx: ReactZoomPanPinchRef) => void> = new Set();
  ...
  initializeWindowEvents = (): void => {
    ...
    this.wrapperComponent?.addEventListener('wheel', this.onWheelPanning, passive);
  ...
  onWheelPanning = (event: WheelEvent): void => {
    ...
    const paddingValueX = getPaddingValue(this, sizeX ?? 0);
    const paddingValueY = getPaddingValue(this, sizeY ?? 0);

    if (newPositionX === positionX && newPositionY === positionY) {
      return;
    }

    handleNewPosition(this, newPositionX, newPositionY, paddingValueX, paddingValueY);
  };
  ...
  onPanningStart = (event: MouseEvent): void => {
    ...
    if (event.button === 0 && !this.setup.panning.allowLeftClickPan) {
      return;
    }

    if (event.button === 1 && !this.setup.panning.allowMiddleClickPan) {
      return;
    }

    if (event.button === 2 && !this.setup.panning.allowRightClickPan) {
      return;
    }

...
export const isPinchStartAllowed = (contextInstance: ReactZoomPanPinchContext, event: TouchEvent): boolean => {
...
export const isPinchAllowed = (contextInstance: ReactZoomPanPinchContext): boolean => {
...
export const getTouchDistance = (event: TouchEvent): number => {
  return Math.sqrt(
    (event.touches[0].pageX - event.touches[1].pageX) ** 2 + (event.touches[0].pageY - event.touches[1].pageY) ** 2
  );
};

export const calculatePinchZoom = (contextInstance: ReactZoomPanPinchContext, currentDistance: number): number => {
  ...
  return checkZoomBounds(roundNumber(scaleDifference, 2), minScale, maxScale, size ?? 0, !disabled && !disablePadding);
};

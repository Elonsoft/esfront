...
export const createSetup = (props: ReactZoomPanPinchProps): LibrarySetup => {
  ...
  Object.keys(props).forEach((key) => {
    const validValue = typeof props?.[key as keyof ReactZoomPanPinchProps] !== 'undefined';
    const validParameter = typeof initialSetup[key as keyof LibrarySetup] !== 'undefined';

    if (validParameter && validValue) {
      const dataType = Object.prototype.toString.call(initialSetup[key as keyof LibrarySetup]);

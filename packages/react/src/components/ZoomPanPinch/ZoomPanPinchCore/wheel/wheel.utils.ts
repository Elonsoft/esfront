...
export const isWheelAllowed = (contextInstance: ReactZoomPanPinchContext, event: WheelEvent): boolean => {
  const { disabled, wheelDisabled, touchPadDisabled, excluded } = contextInstance.setup.wheel;
...
export function getDelta(event: WheelEvent, customDelta?: number | null): number {
...
export const handleWheelZoomStop = (contextInstance: ReactZoomPanPinchContext, event: WheelEvent): boolean => {

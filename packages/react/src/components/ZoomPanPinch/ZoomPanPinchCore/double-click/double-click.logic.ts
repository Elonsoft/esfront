...
function getDoubleClickScale(mode: LibrarySetup['doubleClick']['mode'], scale: number) {
  if (mode === 'toggle') {
    return scale === 1 ? 1 : -1;
  }
...
export function handleDoubleClick(contextInstance: ReactZoomPanPinchContext, event: MouseEvent | TouchEvent): void {
  const { setup, doubleClickStopEventTimer, transformState, contentComponent } = contextInstance;

  const { scale } = transformState;
  const { onZoomStart, onZoom } = contextInstance.props;
  const { disabled, mode, step, animationTime, animationType } = setup.doubleClick;
  ...
  const targetState = handleZoomToPoint(contextInstance, newScale, mousePosition.x, mousePosition.y);

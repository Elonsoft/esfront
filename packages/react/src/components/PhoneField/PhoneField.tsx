  ...
  const onBeforeInput = useEvent((event: InputEvent) => {
    ...
    switch (event.inputType) {
      ...
      case 'deleteByCut': {
        ...
        if (whitelist) {
          ...
          if (codesPartialRegExp.test(phone)) {
            if (!codesRegExp.test(phone)) {
              newData.value = '+';
              newData.selectionStart = 1;
              newData.selectionEnd = 1;
            }
          } else {
            return;
          }
      ...
      case 'insertText': {
        ...
        if (whitelist) {
          if (codesPartialRegExp.test(newData.value)) {
            if (!codesRegExp.test(newData.value)) {
              const possibleCodes = codes.filter((code) => `+${code}`.startsWith(newData!.value));

              if (possibleCodes.length === 1) {
                newData.value = `+${possibleCodes[0]}`;
                newData.selectionStart = possibleCodes[0].length + 1;
                newData.selectionEnd = possibleCodes[0].length + 1;
              }
            }
          } else {
            return;

...
export const FiltersFormGroup = (inProps: FiltersFormGroupProps) => {
  ...
  return (
    <FiltersFormGroupRoot className={clsx(classes.root, className)} sx={sx}>
      {!!header && <FiltersFormGroupHeader>{header}</FiltersFormGroupHeader>}
      <FiltersFormGroupContent ref={ref} ownerState={ownerState} onScroll={onScroll}>
        {Children.map(children, (child, index: number) => {
          if (maxLines && !open && index > maxLines - 1) {
            return null;
          }
